...
def calculate_chromosome_lengths(fasta_file):
    """
    Calculate the length of each chromosome/sequence in a FASTA file.
    ...
    with open(fasta_file, 'r') as file:
        for line in file:
            ...
            if line.startswith('>'):
                ...
                # Start new chromosome - take only first element before whitespace
                current_chromosome = line[1:].split()[0]  # Remove '>' and take first element
                current_length = 0
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total", action='store_true', 
        help="Print total length.")
    
    parser.add_argument("--chrom_lengths_out", type=str, default=None,
        help="Output TSV file with chromosome lengths (chrom<tab>length format).")
    ...
    # Print individual chromosomes within length range
    total_length = 0
    ...
    # Write chromosome lengths to TSV file if requested
    if args.chrom_lengths_out:
        with open(args.chrom_lengths_out, 'w') as f:
            for chromosome, length in chromosome_lengths.items():
