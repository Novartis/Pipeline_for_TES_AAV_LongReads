...
def parse_minimap_settings():
    ...
    for k in required_keys:
        if not k in config["minimap_settings"]:
            raise Exception(f"Key missing under \"minimap_settings\" in config file: {k}")

    if config["minimap_settings"]["shortest_segment"] < 25:
        raise Exception(f"The shortest aligned segment (\"shortest_segment\" under \"minimap_settings\") must be 25 or more.")
    elif config["minimap_settings"]["shortest_segment"] < 100:
        sgs = config["minimap_settings"]["shortest_segment"]
        config["minimap_settings"]["m"] = sgs
        config["minimap_settings"]["s"] = sgs
    else:
        sgs = 100
        config["minimap_settings"]["m"] = sgs
        config["minimap_settings"]["s"] = sgs
...
def required_config_keys():
    required_keys = ["ref_genome", "ref_mmi", "minimap_settings"]
    for k in required_keys:
        if not k in config:
...
def get_plasmid_chr_name(wildcards):
    # Extract all chromosome names (fasta entry IDs) from the plasmid fasta file:
    mask = (ss_df["sample_name"] == wildcards.sample)
    fasta_path = ss_df[mask]["plasmid_seq"].values[0]
    chr_names = list()
    with open(fasta_path) as fh_in:
        for line in fh_in:
            if line.startswith('>'):
                chr_names.append(line[1:].split()[0])
    return(chr_names)
